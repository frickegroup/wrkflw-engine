...
export default class Node {
	...
	async listen<T extends object>(
		...
		const AMQP_CONSUMER = await AMQP_CHANNEL.basicConsume(
			opts.queue,
			{ noAck: false, tag: opts.queue },
			(msg) => {
				...
				if (msg.properties.contentType !== 'application/json')
					throw new Error('message_content must be of type application/json');
				if (msg.bodySize === 0) throw new Error('message cannot be empty');
				const message_content = JSON.parse(msg.bodyToString() ?? '{}') as T;

				if (message_id === undefined)
					throw new Error('message_id cannot be undefined, please set it on publish');
				if (message_timestamp === undefined)
					throw new Error('message_timestamp cannot be undefined, please set it on publish');
